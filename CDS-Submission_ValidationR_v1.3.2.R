...
#Cancer Data Services - Submission Validation-R v1.3.2
...
#This takes an XLSX file as input that is formatted to submission for CDS v1.3.

#Run the following command in a terminal where R is installed.

#Rscript --vanilla CDS-Submission_ValidationR.R input_file.xlsx
...
# test if there is only one argument: if not, stop and return an error
if (length(args)==0) {
  stop("Must supply the script with an input file.", call.=FALSE)
} else if (length(args)>1) {
  stop("Must supply the script with only one input file.", call.=FALSE)
}
...
list_of_packages=c("tidyverse","readr","stringi","janitor","readxl")
...
cat("The XLSX file is being validated at this time.\n")
...
if (all(expected_sheets%in%sheet_names)){
  cat("PASS: Found expected sheets in the submitted XLSX file.\n")
  }else{
    sheet_gone=1
  }
...
df_dict=suppressMessages(read_xlsx(path =file_path,sheet = "Dictionary"))
...
#Read in metadata page to check against the expected/required properties.
...
df_tavs=suppressMessages(read_xlsx(path = file_path,sheet = "Terms and Value Sets"))
...
for (x in 1:length(VSN)){
  if (!is.na(VSN[x+1])){
    df_create=tibble(x=df_tavs[VSN[x]:(VSN[x+1])-1,3])
    colnames(df_create)<-df_tavs[VSN[x],1]
    df_all_terms[colnames(df_create)] = as.vector(df_create)
  }else{
    df_create=tibble(x=df_tavs[VSN[x]:dim(df_tavs)[1],3])
    colnames(df_create)<-df_tavs[VSN[x],1]
    df_all_terms[colnames(df_create)] = as.vector(df_create)
  }
}
