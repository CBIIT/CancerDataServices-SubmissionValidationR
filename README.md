...
|ERROR: The Metadata sheet is missing the following required column: {column}|The template that has metadata entered is missing the specified columns.|Obtain the [newest version of the template](https://github.com/CBIIT/cds-model/tree/main/metadata-manifest), and migrate the metadata into the newly acquired template.|
|ERROR: Column from a required group of properties, {required_property_group}, was found empty on the Metadata sheet. Please add values for the following columns: {column} |For groups (Study, Sample, Participant, etc) that have information in some of the required properties, there are other required properties that are missing values.|Obtain the information for all required columns in a given row for a group.|
...
|ERROR: {value_set_name} property contains a value that is not recognized: {value}|The property has an expected enumeration list and the value that was supplied does not match that list.|Locate the "Terms and Value Sets" tab in the template, and determine the given acceptable values for the property.|
